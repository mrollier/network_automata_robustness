...
import h5py  # compact saving of data
import igraph as ig
import numpy as np
import torch as tc
...
def use_latex_style():
    """Enable LaTeX text rendering with Times New Roman (needs a TeX install)."""
    rcParams.update(
        {
            "text.usetex": True,
            "font.family": "serif",
...
def parse_arguments():
    ...
    parser.add_argument(
        "--rewiring_prob", type=float, default=0.2, help="Rewiring probability for the small-world network."
    )
    parser.add_argument(
        "--degree", type=int, default=8, help="Degree for lattice model (Moore neighbourhood)."
    )
    parser.add_argument("--num_graphs", type=int, default=30, help="Number of graphs per type.")
    parser.add_argument(
        "--num_init_conf", type=int, default=30, help="Number of initial configurations per graph."
    )
    ...
    parser.add_argument(
        "--output_file",
        type=str,
        default="state-median-vs-defect-median.h5",
        help="Output file name for saving results.",
    )
    parser.add_argument(
        "--network_type",
        type=str,
        choices=["toroidal-lattice", "small-world", "random"],
        required=True,
...
def main():
    ...
    logging.info(
        f"Number of graphs: {num_graphs_per_type}, Initial configurations per graph: {num_init_conf_per_graph}"
    )
    ...
    if network_type == "toroidal-lattice":
        ...
        raise ValueError(
            f"Invalid network type: {network_type}. Must be one of 'toroidal-lattice', 'small-world', or 'random'."
        )
    ...
    logging.info(
        f"Running through the non-equivalent rules of resolution {resolution} for {network_type} network..."
    )
    # loop over rules
    for i, (beta, sigma) in enumerate(beta_sigma_list):
        ...
        init_configs_array = init_config_with_dens(
            num_nodes * num_init_conf_per_graph * num_graphs_per_type, init_dens
        )
        init_configs_array = init_configs_array.reshape(
            num_graphs_per_type, num_init_conf_per_graph, num_nodes
        )
        # get initial defects
        init_defects_array = np.eye(num_nodes, dtype=int)[
            np.random.choice(num_nodes, num_init_conf_per_graph * num_graphs_per_type, replace=False)
        ]
        init_defects_array = init_defects_array.reshape(
            num_graphs_per_type, num_init_conf_per_graph, num_nodes
        )
        ...
        # loop over graphs
        for edge, init_configs, init_configs_with_defect in zip(
            edges, init_configs_array, init_configs_with_defect_array, strict=True
        ):
            # evolve the configuration
            configs = model.forward(edge, tc.tensor(init_configs), T=T).numpy().astype(int)
            configs_with_defect = (
                model.forward(edge, tc.tensor(init_configs_with_defect), T=T).numpy().astype(int)
            )
        ...
        state_median, state_q1, state_q3 = median_and_percentiles_over_ensemble(
            state_averages_stack, delta_t=delta_t, time_axis=1
        )
        defect_median, defect_q1, defect_q3 = median_and_percentiles_over_ensemble(
            defect_averages_stack, delta_t=delta_t, time_axis=1
        )
        # append to lists
    ...
    logging.info(
        f"Saving results to {args.output_file} in {'append' if file_mode == 'a' else 'write'} mode..."
    )
